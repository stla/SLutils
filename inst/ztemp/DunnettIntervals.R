...
predictionInterval <- function(x, n.mean = 1, k = 1, method = "Bonferroni",
  ...
  if(method=="SL"){
    K <- predictionFactor_SL(n=length(x), n.mean=n.mean, k=k,
                             method="exact", pi.type=pi.type,
...
Dunnetts_K_SL <- function (n, df = n-1L, k, m=1, method = c("Bonferroni", "exact"),
                           type = c("two-sided", "upper", "lower"), conf.level = 0.95)
{
  method <- match.arg(method)
  type <- match.arg(type)
  ...
  if (k > 1 && method == "exact") {
    ...
    # d <- optim(par = K/sqrt(1/m + 1/n), fn = fcn.to.min,
...
predictionFactor_SL <- function(n, df = n - 1, n.mean = 1, k = 1,
                                method = c("Bonferroni", "exact"),
                                pi.type = "two-sided",
                                conf.level = 0.95){
  if (!is.vector(n, mode = "numeric") || length(n) != 1 ||
  ...
  method <- match.arg(method, c("Bonferroni", "exact"))
  pi.type <- match.arg(pi.type, c("two-sided", "lower", "upper"))
  Dunnetts_K_SL(n = n, df = df, k = k, m = n.mean, method = method,
                type = pi.type, conf.level = conf.level)
}
